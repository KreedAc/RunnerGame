...
/* -------------------------------- EROE -------------------------------- */
/* Vichingo: elmo con corna, tunica, barba. La silhouette si riconosce
   anche a venti blocchi di distanza, che è quanto serve. */
function buildHero() {
  const g = new THREE.Group();
  const skin  = mat(C.skin);
  const cloth = mat(C.cloth);
  const fur   = mat(C.hero);
  const dark  = mat(C.heroDark);
  const metal = mat(0xc3ccd6);

  const legL = limb(g, dark, -0.19, 0.78, 0, 0.26, 0.78);
  const legR = limb(g, dark,  0.19, 0.78, 0, 0.26, 0.78);
  put(legL, GEO.box, mat(0x4a3626), 0, -0.76, 0.05, 0.3, 0.16, 0.4);
  put(legR, GEO.box, mat(0x4a3626), 0, -0.76, 0.05, 0.3, 0.16, 0.4);

  put(g, GEO.taper, cloth, 0, 1.18, 0, 0.92, 0.92, 0.66);   // tunica
  put(g, GEO.cyl,   dark,  0, 0.80, 0, 0.86, 0.16, 0.62);   // cintura
  put(g, GEO.cyl,   mat(C.gold), 0, 0.80, 0.30, 0.20, 0.20, 0.10);
  put(g, GEO.sph,   fur,   0, 1.55, 0, 1.00, 0.42, 0.72);   // spallacci di pelliccia

  const armL = limb(g, skin, -0.46, 1.46, 0, 0.24, 0.72);
  const armR = limb(g, skin,  0.46, 1.46, 0, 0.24, 0.72);

  put(g, GEO.sph, skin, 0, 1.90, 0, 0.56, 0.58, 0.54);      // testa
  put(g, GEO.sph, mat(0xd9c4a8), 0, 1.79, 0.20, 0.44, 0.40, 0.30); // barba
  eyes(g, 1.96, 0.24, 0.13, 0.10);

  // elmo con corna
  const helm = put(g, GEO.sph, metal, 0, 2.02, 0, 0.62, 0.52, 0.60);
  helm.geometry = GEO.sph;
  put(g, GEO.cyl, metal, 0, 1.92, 0, 0.64, 0.16, 0.62);
  for (const s of [-1, 1]) {
    const horn = put(g, GEO.cone6, mat(0xe6dcc8), s * 0.36, 2.10, 0, 0.20, 0.42, 0.20);
